...
const wallet = new ethers.Wallet(process.env.USER_PRIVATE_KEY as string, provider)
...
const lendingContract = getContract(addresses.lending, wallet, lendingAbi);
const stableContract = getContract(addresses.stable, wallet, mockStableAbi);

const output = await collateralContract.getApproved("0x2");
